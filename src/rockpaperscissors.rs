...
mod tests {
    ...
    fn test_rock_paper_scissors_round() {
        ...
        assert_eq!(
            play_round(&Action::Rock, &Action::Scissors),
            RoundResult::Loss
        );
        assert_eq!(play_round(&Action::Paper, &Action::Rock), RoundResult::Loss);
        assert_eq!(
            play_round(&Action::Paper, &Action::Paper),
            RoundResult::Draw
        );
        assert_eq!(
            play_round(&Action::Paper, &Action::Scissors),
            RoundResult::Win
        );
        assert_eq!(
            play_round(&Action::Scissors, &Action::Rock),
            RoundResult::Win
        );
        assert_eq!(
            play_round(&Action::Scissors, &Action::Paper),
            RoundResult::Loss
        );
        assert_eq!(
            play_round(&Action::Scissors, &Action::Scissors),
            RoundResult::Draw
        );
    }
}
