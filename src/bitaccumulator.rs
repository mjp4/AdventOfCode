...
impl DiagsReport {
    ...
    fn accumulate_bits(&self) -> BitAccumulator {
        self.values.iter().fold(
            BitAccumulator::zeroed(self.digit_count),
            |ba, bits| ba.accumulate(*bits)
        )
    }
    ...
    pub fn oxygen_rate(&self) -> usize {
        (0..self.digit_count).rev().fold::<DiagsReport, _>(
            self.cloned(),
            |filtered_report, index| {
                ...
                if filtered_report.values.len() == 1 {
                    ...
                    DiagsReport::from_iter(
                        index,
                        filtered_report.iter_values().filter(
                            |v| bit_match_at_index(index, ba.most_common_bits(), **v)
                        ).cloned())
                }
            }
        ).values[0]
            ...
            |filtered_report, index| {
                ...
                if filtered_report.values.len() == 1 {
                    ...
                    DiagsReport::from_iter(
                        index,
                        filtered_report.iter_values().filter(
                            |v| bit_match_at_index(index, ba.least_common_bits(), **v)
                        ).cloned())
                }
            }
        ).values[0]
